...
hand_cascade = cv2.CascadeClassifier('data/Hand_haar_cascade.xml')
...
while (True):
	...
	retval2,thresh1 = cv2.threshold(gray,40,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU) # THRESHOLDING IMAGE
	...
	if len(contours) > 0:
		...
		if defects is not None:
			for i in range(defects.shape[0]):
				...
				dip = tuple(cnt[r][0])
				cv2.line(img,finger1,finger2,[0,255,0],2)
		...
		if count_defects == 1:
			cv2.putText(img,"c'est 2", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
		elif count_defects == 2:
			cv2.putText(img,"c'est 3", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
		elif count_defects == 3:
			cv2.putText(img,"c'est 4", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
		elif count_defects == 4:
			cv2.putText(img,"c'est 5", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
			
	#cv2.imshow('img',thresh1)
	all_image = np.hstack((thresh1, img2))
	cv2.imshow('img1',img)
	cv2.imshow('img2',all_image)
